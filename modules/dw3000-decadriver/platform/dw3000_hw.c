...
{
	/* Reset. LOCAL DELTA vs upstream: driven push-pull, not released as
	 * GPIO_INPUT. On ancla_esp32s3 this pin drives a transistor base, not the
	 * DW3220 RSTn pad, so there is no internal pull-up to release it to —
	 * GPIO_INPUT would leave the base floating. Driving it is safe here for the
	 * same reason (Qorvo's "never drive RSTn high" rule applies to the pad, and
	 * this pin is not connected to it). */
...
void dw3000_hw_reset()
{
	...
	/* LOCAL DELTA vs upstream: release by driving inactive, not GPIO_INPUT —
	 * see the comment in dw3000_hw_init(). */
