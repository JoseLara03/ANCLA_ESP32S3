...
### Task 3: Lift the deployment cap

- [ ] `src/uwb_config.h`: `UWB_MAX_ANCHORS` 4 -> 32. Add a comment naming the **wire cap
...
- [ ] `src/anchor_shell.c`: `anchor id <0..31>`, and the help text with it.
- [ ] `tests/uwb_config/`: accept 31, reject 32; the short-address mapping stays
      `0x0001 + id`.
- [ ] Verify `apos_node.c`'s `RANGE_CMD` peer range check follows `UWB_MAX_ANCHORS`
      rather than a literal.

### Task 4: Grouped DISCOVERY responder

- [ ] Copy the tag's updated `uwb_frame_802_15_4z.{c,h}` (tag Task 3) and run both
      repos' `tests/uwb_frame/`.
- [ ] `src/disc_schedule.{c,h}`: add
...
- [ ] Keep `disc_resp_delay_uus()` for `n_groups == 1` compatibility, or delete it and
      update every caller — do not leave two schedules that can disagree.
- [ ] **Bound the rank.** With `n_groups` under-sized for the deployment, `rank` exceeds
...
- [ ] `src/anchor_respond.c`: parse `group`/`n_groups`, answer only on a match, use the
...
- [ ] `tests/disc_schedule/`: every (id, n_groups) pair for 32 anchors matches exactly
      one group; max delay over all ids at `n_groups = 8` is 12500 uus; the `rank > 3`
      refusal fires.

### Task 5: ANNOUNCE (`0xEC`) emitter

- [ ] Copy the tag's frame module again (tag Task 4); run both suites.
- [ ] `src/uwb_slave.c`: after receiving a beacon, if `beacon.announce_id == anchor_id`,
      transmit one `0xEC` at a fixed delay inside the announce window, carrying the short
      address, `(x, y, z)` and the CIR quality of that beacon.
- [ ] Gate it on `position_valid` exactly like the WAVE responder — an unpositioned board
      must not advertise coordinates it does not have.
- [ ] Route it through `beacon_guard` like every other delayed TX.
- [ ] `z` comes from `anchor pos <x> <y> <z>`, which already exists and is already
      surveyed by `apos`; emit NaN when the anchor has no height rather than 0.0, so the
      tag can fall back rather than trusting a floor-level lie.
...
### Task 6: Gateway schedules the announce

- [ ] `src/uwb_gateway.c`: beacon gains `announce_id = frame_counter % A` with
      **`A = 37`**, prime and coprime with the cycle `C = 16`.
- [ ] Put the coprimality requirement in a comment *and* in a host test: with `A = 32` a
...
- [ ] `A` lives next to `C` in one header so the two cannot drift into a common factor.
- [ ] `tests/gw_core/`: for every phase `p` in `0..15`, the set of announce ids observed
      by a tag sampling that phase covers all `0..A-1`.

### Task 7: Survey scales to 32 (sparse mesh)

- [ ] `src/apos_geom.h`: `APOS_MAX_NODES` 8 -> 32. Note the memory: `APOS_MAX_EDGES`
      becomes 496 and `APOS_MAX_MEAS` 992 — size the structs deliberately, do not let
      them land on the gateway loop's stack.
- [ ] **Candidate pairs only.** `apos_table` records which peers heard each other during
...
- [ ] **Rigidity check before trusting a solve.** A sparse framework can be flexible or
...
- [ ] `rms_mm` becomes a real acceptance signal at 32 sparse-but-redundant nodes — but
...
- [ ] `apos enum` stagger must fit 32 responders: reuse the EUI-hash stagger with enough
      slots, or run enumeration in rounds like grouped discovery. Whichever, bound the
      window and say what bounds it.
- [ ] `pos_json_anchors()` at 32 nodes is ~4.8 kB — check it against the MQTT buffer and
      chunk or truncate deliberately rather than overflowing.
- [ ] `tests/apos_geom/`, `tests/apos_table/`: a sparse 12-node mesh solves; a
...
### Task 8: MPOL_RESP (`0xED`)

- [ ] Copy the tag's frame module (tag Task 7); run both suites.
- [ ] Confirm the byte layout against the tag's parser with a shared vector — this frame
      carries timestamps, so a silent field-offset disagreement becomes a wrong distance,
      not a dropped frame.

### Task 9: Answer `0xE3` MULTI-POLL

- [ ] `src/anchor_respond.c`: new `anchor_respond_multipoll()`. Parse the slot list,
...
- [ ] Ignore a poll that does not name us. Log it at DBG the way `wave_other` does; that
      line is the first fork when diagnosing a silent anchor.
- [ ] `beacon_guard` applies per response exactly as for WAVE.
- [ ] `TX_COMPLETE_TIMEOUT_MS` on this path must cover the **largest** `delay_us` the tag
      can assign (`MPOL_BASE_UUS + 3 * MPOL_SLOT_UUS` ~ 7.1 ms) plus airtime. State the
      derivation in the comment. Do not reuse the DISCOVERY constant by accident.
- [ ] Keep the WAVE single-poll responder compiled and working — the calibration image
      and the bench depend on it.
...
### Task 12: Seat table gains phases

- [ ] `src/gw_core.h`: `GW_CYCLE_C` = 16; `seats[GW_CYCLE_C][GW_N_CFP]`. At 14 slots that
      is 224 seats; check the struct size against available RAM and keep it out of the
      gateway loop's stack.
- [ ] `gw_core_build_slotmap(c, phase, out)` publishes one phase's row.
- [ ] `gw_core_superframe_tick()` ages leases once per superframe for **all** phases, not
      only the current one. A tag that owns one phase must not age 16x slower than one
      that owns four.
- [ ] `tests/gw_core/`: 224 distinct tags each get a seat; the 225th is refused cleanly;
...
- [ ] `gw_core_join()` grants one phase by default; `gw_core_keepalive()` /
      the POS path re-grants `n_phases` from the tag's requested tier: IDLE 1, SLOW 2,
      FAST 4 (8 when the budget allows).
- [ ] Allocation spreads a mover's phases **evenly** around the cycle (`phase, phase+C/n,
...
- [ ] Under contention, a FAST tag never takes the *last* phase of a stationary tag: an
      idle tag with zero phases has silently lost its seat.
- [ ] `tests/gw_core/`: even spread for 2, 4 and 8 phases; contention never strands a tag
      at zero phases; a tier drop returns phases to the pool.

### Task 14: GRANT carries the phase mask

- [ ] Frame-module copy from the tag (tag Task 11): GRANT 24 -> 26 with a 16-bit
      `phase_mask`. Run both suites.
- [ ] `send_grant()` fills it from the allocator.
- [ ] A re-tier that changes only the mask is published in the next GRANT, not by
      forcing a re-JOIN.

### Task 15: POS renews the lease; JOIN backoff

- [ ] `uwb_gateway.c`'s POS dispatch already decodes `0xEA` and looks up the seat by
...
- [ ] `tests/gw_core/`: `gw_core_pos_seen(addr)` refreshes exactly the matching seat, does
      nothing for an unknown address, and never creates a seat.
- [ ] JOIN backoff: the GRANT (or a beacon field) tells a refused tag to retry after a
      random 0-63 superframes. 100 tags rejoining after a gateway restart otherwise
      collide in 4 mini-slots indefinitely.

### Task 16: Short addresses are reused by EUI

- [ ] `alloc_short_addr()` keeps an EUI -> address map (64 entries, LRU) so a rejoining
      tag gets its previous address back.
- [ ] This also removes the `Tid` phantom-device case documented in `CLAUDE.md`. Keep the
      EUI-hash `Tid` regardless — it is the stable identity and does not depend on this.
- [ ] `tests/gw_core/`: rejoin after lease expiry returns the same address; the map
      evicts oldest-first; a full map still grants a fresh address rather than failing.

### Task 17: `gw seed <n>` — synthetic full occupancy

- [ ] Console command filling `n` synthetic seats with fabricated EUIs, so the real
...
- [ ] Bump `UWB_PROTO_VER` in the frame module (tag repo lands it, this repo copies) and
      reflash the **whole** fleet in one session: gateway, every anchor, every tag.
